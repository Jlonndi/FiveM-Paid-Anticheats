...
BlacklistedCmdsxd = {"chocolate","pk","haha","lol","panickey","killmenu","panik","ssssss","brutan","panic","desudo","jd","ham","hammafia","hamhaxia","redstonia, xariesmenu, MainMenu, SelfMenu"}
    ...
    while true do
		...
		for _, bcmds in ipairs(BlacklistedCmdsxd) do
				if bcmd.name == bcmds then
					TriggerServerEvent("AC:ViolationDetected","⚡️ Injection detected!",true)
			end
		end
		end
	end
		...
		while true do
			...
			if Config.AntiKeyInsert then
				if IsControlJustReleased(0, 121) then
					TriggerServerEvent("AC:ViolationDetected","⚡️ Insert Blacklisted Key",true)
				end
			end
			if Config.AntiKeyTabQ then
				if IsDisabledControlPressed(0, 37) and IsDisabledControlPressed(0, 44) then
					TriggerServerEvent("AC:ViolationDetected","⚡️ Tab+Q Blacklisted Key",true)
				end
			end
			if Config.AntiKeyShiftG then
				if IsDisabledControlPressed(0, 47) and IsDisabledControlPressed(0, 21) then
					TriggerServerEvent("AC:ViolationDetected","⚡️ Shift+G Blacklisted Key",true)
				end
			end
		end
	end)
